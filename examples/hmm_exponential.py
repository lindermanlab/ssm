...
#log_lambdas = true_hmm.observations.params
#fit_log_lambdas = model.observations.params
#for state in range(log_lambdas.shape[0]):
#    for dim in range(log_lambdas.shape[1]):
#        la = np.exp(log_lambdas[state,dim])
#        fit_la = np.exp(fit_log_lambdas[state,dim])
#        fexp = la*np.exp(-la*x)
#        fit_fexp = fit_la*np.exp(-fit_la*x)
#        plt.figure()
#        plt.title('State {} - dim {}'.format(state,dim))
#        plt.hist(y[z==state,dim],bins=20,density=True,alpha=0.5)
#        plt.plot(x,fexp,label='real distribution')
#        plt.legend()
#        plt.figure()
#        plt.title('State {} - dim {}'.format(state,dim))
#        plt.hist(smoothed_y[smoothed_z==state,dim],bins=20,density=True,alpha=0.5)
#        plt.plot(x,fit_fexp,label='fitted distribution')
#        plt.legend()
#        
#
#x = np.arange(0,10,0.1)
...
#plt.figure()
#for state in range(log_lambdas.shape[0]):
#    for dim in range(log_lambdas.shape[1])[:1]:
#        la = np.exp(log_lambdas[state,dim])
##        fit_la = np.exp(fit_log_lambdas[state,dim])
#        fexp = la*np.exp(-la*x)
##        fit_fexp = fit_la*np.exp(-fit_la*x)
#        plt.plot(x,fexp,label='state = {}'.format(state))
#        plt.legend()
